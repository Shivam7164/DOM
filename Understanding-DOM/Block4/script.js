// Give three examples of BOM features.
```js
screen
location
navigator
```
// What is DOM?
```js
DOM stands for Document Object Model. 
It is a programming interface that allows us to create, change, or remove elements from the document. 
We can also add events to these elements to make our page more dynamic.
```
// Convert the following HTML into DOM (object form) <p class="para">Hello User!</p>
```js
{
    tag:"p",
    innerText:"Hello User!",
    className:"para"
}
```
// Convert the following HTML into DOM <div> <p class="para">Hello User!</p> </div>
```js
{
    tag:"div",
    innerText:"Hello User!",
    className:"para"
}
```